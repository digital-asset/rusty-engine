...
impl<'a, 'store> State<'a, 'store> {
    ...
    fn start_span(&mut self, label: &'a str) {
    ...
    fn step_expr(&mut self, ctrl_expr: &'a Expr) -> Ctrl<'a> {
        match ctrl_expr {
            ...
            Expr::Let {
                ...
                self.kont.push(Kont::Label(binder));
                Ctrl::Expr(bound)
            }
    ...
    fn interpret_prim(&mut self, prim: Prim<'a>, args: Vec<Rc<Value<'a>>>) -> Ctrl<'a> {
        match prim {
            ...
            Prim::Lam(body, label, captured) => {
                self.start_span(label.unwrap_or("<anonymous lambda>"));
